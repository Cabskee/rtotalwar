...
function createFlair(flairParent, flair, sectionContent) {
    var newFlairParent = $('<div class="col-4 flair-parent"></div>');
    var newFlair = $('<span class="flair"></span>');
...
function updateFlair(flairClasses, flairText) {
    ...
    var chosenFlair = parseFlair($('#id-user-flair').prop('class'));
        ...
        for (var gameKey in responseData) {
            ...
            for (var flairsKey in responseData[gameKey]) {
                ...
                if (responseData[gameKey][flairsKey]['message']) {
                    var sectionMessage = $('<h5>'+responseData[gameKey][flairsKey]['message']+'</h5>');
                    gameTabContent.append(sectionMessage);
                    continue;
                }

                // Set subtitle content
                var sectionContent = $('<div class="row no-gutters mb-4"></div>');

                $.each(responseData[gameKey][flairsKey]['flairs'], function(index, flair) {
                    if (is.not.existy(flair['class'])) {
                        var subsectionContent = $('<div class="row no-gutters col-12 py-3"></div>');
                        sectionContent.append(subsectionContent);

                        var subsectionTitle = $('<h5 class="col-12">'+toTitleCase(flair['title'])+'</h5>');
                        if (flair['subtitle'])
